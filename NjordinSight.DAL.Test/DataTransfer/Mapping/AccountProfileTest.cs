...
namespace NjordinSight.Test.DataTransfer.Mapping
{
    ...
    public class AccountProfileTest : IProfileTest
    {
        ...
        [TestMethod]
        public void Account_MapFrom_AccountDto_WithAddedAttributes_YieldsPlusOneAttributeEntry()
        {
            // Arrange
            var accountDto = new AccountDto()
            {
            ...
            var initialCountPlusOne = accountDto.Attributes.Count + 1;
            ...
            // Fact: Attributes property is non-empty collection with count matching source 
            //       and expected count from the arrangment.
            Assert.AreEqual(
                accountDto.Attributes.Count,
                account.AccountNavigation.AccountAttributeMemberEntries.Count);

            Assert.AreEqual(
                expected: initialCountPlusOne,
                actual: account.AccountNavigation.AccountAttributeMemberEntries.Count);
        }

        [TestMethod]
        public void Account_MapFrom_AccountDto_WithDeletedAttributes_YieldsMinusOneAttributeEntry()
        {
            // Arrange
            var testAttributeDto = new AccountBaseAttributeDto()
            {
                AccountObjectId = 1,
                AttributeMember = new()
                {
            ...
            var initialCountMinusOne = accountDto.Attributes.Count - 1;
            ...
            // Act
            accountDto.AttributeCollection.EntryCollection.First().RemoveEntry(testAttributeDto);
            var account = mapper.Map<Account>(accountDto);
            ...
            Assert.AreEqual(
                expected: initialCountMinusOne,
                actual: account.AccountNavigation.AccountAttributeMemberEntries.Count);
        }

        [TestMethod]
        public void Account_MapFrom_AccountDto_WithModifiedAttribute_YieldsModifiedAttributeEntry()
        {
            // Arrange
            var accountDto = new AccountDto()
            {
            ...
            // Act
            accountDto.AttributeCollection.CurrentEntries.First().AttributeMember = new()
            {
                AttributeMemberId = 3,
            ...
            Assert.IsTrue(account.AccountNavigation
                .AccountAttributeMemberEntries
                .Any(x => x.AttributeMemberId == 3));
        }
    }
}
